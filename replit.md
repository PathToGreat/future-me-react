...
-   **Body Model Integrity & Cross-Gender Robustness (Phase 20):** Replaces composite identity score with dedicated `PhysicalCompositionScore` (activity 40%, nutrition 25%, consistency 25%, sleep 10%) for body shape — emotional traits only drive posture, facial tension, vibrancy, and glow. `avatarPresets.js` defines 5 ordered tiers (Overweight→Soft→AverageFit→LeanAthletic→MuscularAthletic) with `TIER_BOUNDARIES` at 0/20/40/60/80/100 and `getTierInterpolation()` for clean linear blending between adjacent tiers only. Per-preset `FEMALE_ADJUSTMENTS` preserve muscular potential (female MuscularAthletic retains 96% shoulder width, 97% arm thickness). `bodyConstraints.js` enforces hard relational rules: shoulders > neck, waist bounded by hips, arms proportional to chest, legs proportional to hips, head scale capped at 0.58. `validateBodyModel()` confirms all 10 gender×score combinations pass structural integrity checks with body unchanged by emotional state variation.
-   **Projection Credibility & Release Stabilization (Phase 21):** `projectionConfidence.js` computes confidence tiers (LOW <14 days, MEDIUM 14-30 days, HIGH 30+ days with consistent logging). LOW limits body deltas to ±8%, dampens emotional projections to 50%, and triggers gentle narrative qualifier tone. MEDIUM allows ±15% deltas with developing-pattern language. HIGH allows full projection effect. `applyConfidenceScaling()` and `applyDeltaVisibilityFloor()` enforce these rules in `mapTraitsToAvatarParams`. `validateProjectionDifference()` validates Current vs Future are neither identical at HIGH confidence nor drastically different at LOW. Narrative engine aligns tone with visual direction (strengthening/resilience/steadiness for gains, softening/reactivity for declines). AvatarScreen shows subtle banner "Future projection is refining as more data is logged" when confidence is LOW. `identityTrajectoryEngine` carries `projectionConfidence` tier through to all consumers. No console logs remain. `USE_HUMAN_AVATAR_V2 = true` across all components. System is stable and publish-ready.
-   **Avatar V2 Visual Clean-up + Calm Face Mapping + Hair Fix + Gender Lock (Phase 22):** Four targeted fixes: (A) Background artifacts — GlowLayer threshold raised to 0.4 (was 0.15), max opacity capped at 0.12 (was 0.25), radius reduced; ShadingLayer opacity reduced to 0.08→0.02 range, waist rect removed; outer aura ellipse removed entirely. (B) Facial tension fix — quadratic curve `rawTension²` replaces linear mapping so stress=2/5 (emotionalStability ~75) produces facialTension ~0.0625 instead of ~0.25; applied in both `computeEmotionalVisualParams` and fallback metrics path; `mapFromAvatarEffects`/`mapFromAvatarEffectsProjected` fixed to use facial expression score directly as emotionalStability (was incorrectly inverted via bracket thresholds). Default facialTension lowered to 0.15. (C) Hair redesign — short hair redrawn as scalp-top contour with natural hairline curve and highlight stroke; medium hair redrawn with side volume flowing below ears, inner hairline cutout, top and side highlight strokes; neither covers eyes or forehead. (D) Gender lock — `GenderSelector` shows locked single-line "Body type: Male/Female" with Edit link when gender is already set; Edit opens selector; changing gender requires Confirm/Cancel step; new users see full selector until first save.
-   **Long Hair + Hair Color Palette (Phase 23):** Adds 4th hair style "Long" extending below shoulders with natural silhouette, visible hairline, and highlight strokes. `HAIR_COLOR_PALETTE` defines 8 colors (Black, Dark Brown, Brown, Light Brown, Dark Blonde, Blonde, Auburn, Gray) with base+highlight pairs. `SkinToneSelector.jsx` exports `loadHairColor()`, `getHairColors()`, and `HAIR_COLOR_PALETTE`; hair color picker appears beneath hair style pills when a style is selected. `hairColor` persisted to localStorage (`futureme_hair_color`), default `darkBrown`. `HumanAvatarRenderer` accepts `hairColor` param, resolves via `getHairColors()`, passes `hairColors` object to `HairLayer`. All avatar components (`FutureMeAvatar`, `FutureAvatar`, `MiniAvatarPreview`) resolve and thread `hairColor` through to `humanAvatarParams`. `AvatarScreen` manages `hairColor` state alongside `skinTone`/`hairStyle`. `VisualInfluences` passes `onHairColorChange` callback.
