...
-   **Multi-Zone Visual Integration Layer:** Avatar rendering pipeline applies subtle visual influences from all 6 Life Zones (Health, Social Emotional, Wealth, Faith, Family, Community) via `zoneInfluenceEngine.js`. Health remains the primary driver of energy color and physical tone. Non-health zones apply small trait offsets (max ±8%): Wealth affects grounding/posture, Social Emotional affects facial expression, Family/Community affect openness, Faith affects steadiness/clarity. Zone influences are applied after health-based traits and baseline modifiers in `FutureMeAvatar`, `FutureAvatar`, `MiniAvatarPreview`, and `useEnhancedAvatarState`. No new colors, icons, or symbolic overlays introduced. Expandable "Visual Influences" section on Avatar screen shows which zones are active and what they influence. If zone data is insufficient, no influence is applied.
-   **Identity Trajectory Engine (ITE):** Foundational predictive architecture (internal, no UI). Derives 7 identity traits (Vitality, Resilience, EmotionalStability, Discipline, Confidence, SocialConnectedness, PurposeAlignment) from raw metrics and life zone scores via a weighted trait mapping table. Each trait carries currentScore, baselineScore, 7-day avg, 30-day avg, velocity (direction + magnitude), 12-month projection, and 5-year projection. Velocity uses conservative smoothing with ±15 cap. Projections apply damping factors and diminishing returns at extremes. Visual delta engine converts projected trait shifts into small scalar adjustments (±0.1) for future avatar rendering. Narrative engine produces grounded, non-hype identity summaries. Master pipeline in `identityTrajectoryEngine.js` orchestrates: state computation → velocity → projections → visual deltas → narrative. Files: `identityTraits.js`, `traitMappingTable.js`, `identityStateEngine.js`, `projectionEngine.js`, `identityVisualDeltaEngine.js`, `identityNarrativeEngine.js`, `identityTrajectoryEngine.js`.
-   **ITE Avatar Routing (Phase 11):** Avatar state is now routed through the Identity Trajectory Engine when sufficient data exists (3+ history entries + baseline). `iteAvatarAdapter.js` converts ITE trait scores into the existing avatar input format (metric proxies on 1-5 scale, wellness score 0-100). Two modes: CURRENT (uses trait currentScores for FutureMeAvatar, MiniAvatarPreview, useEnhancedAvatarState) and FUTURE (uses 12-month projections scaled to 90-day horizon for FutureAvatar). Visual deltas from `identityVisualDeltaEngine.js` are applied as subtle offsets in the adapter layer. Falls back to raw-metric approach for new users without enough data. Zone influences (Phase 9) continue to apply after ITE-based effects. `historyData` prop added to FutureMeAvatar and FutureAvatar; passed from AvatarScreen.
-   **Trait-Based Reflections & Insights (Phase 12):** Shifts reflections and insights from metric reporting to identity trajectory interpretation. `traitInsightsEngine.js` generates insight objects (PATTERN_DETECTED, OBSERVED_CHANGE, DAILY_INSIGHT, CURRENT_FOCUS) framed in identity trait language instead of raw metrics. TodaysReflection.jsx prefers trait-based insights when ITE is available, falling back to metric-based for new users. InsightsFeed.jsx generates and stores trait insights alongside existing metric insights with full deduplication. AvatarScreen description text swaps to ITE narrative (currentSummary, projection12MonthSummary) when data exists. Metrics referenced only as supporting context, not headlines. No new UI, tabs, or sections added.
-   **Action-to-Identity Consequence Wiring (Phase 13):** Maps user actions to identity trait impacts via `actionTraitMappingTable.js` (12 standard actions like SleepConsistencyImproved, LoggingConsistency, FaithPracticeConsistency — each with weighted trait mappings, polarity, time-to-effect, and confidence rules). `traitImpactEngine.js` computes conservative impact estimates and generates consequence lines (e.g., "This incrementally supports Resilience and Discipline."). `loggingConsequenceInference.js` infers 0-3 actions from each daily log based on metric changes vs recent averages and consistency patterns — low-confidence inferences are discarded. TryThis suggestions in trait insights now include linked actionKeys and consequence lines naming the traits they affect. AvatarScreen Future Me narrative appends a "strongest lever" line when an inferred action aligns with the top projected trait shift. No new UI, no gamification.
