...
- **Life Zone System:** Tracks and displays progress across 6 life zones (Health, Social Emotional, Wealth, Faith, Family, Community) with real-time scoring based on daily metrics and habit bonuses. All zones initialize at 50 points for new users; onboarding calculates first real scores. Features include interactive "View Details" modal for each zone, displaying historical daily logs (last 10 entries), daily metric input controls (sliders 1-5 for activity, nutrition, sleep, stress), and immediate score recalculation upon submission. Submissions save to Firestore dailyData collection and trigger atomic updates to user profile and all Life Zone scores.
