...
-   **Avatar Differentiation & Calibration (Phase 26):** Addresses body and face compression across accounts. (A) Preset geometric separation widened — Overweight/Soft use narrower shoulders, larger waist, thicker limbs; LeanAthletic/Muscular use wider shoulders, narrower waist. (B) Tier boundaries adjusted to 0–30 Overweight, 30–50 Soft, 50–70 AverageFit, 70–85 LeanAthletic, 85–100 MuscularAthletic. (C) Face expressiveness increased — brow angle multiplier 3→6, eye openness range 5→3.8 widened to 5.2→3.4, mouth curve range 2.5→-1.5 widened to 3→-2.5. (D) Stress tension multiplier raised by 15% (×1.15). (E) Projection confidence confirmed to only gate FUTURE deltas — CURRENT view already used direct mapping without confidence scaling. (F) `diagnosticAvatarProfile()` diagnostic function exported from `mapTraitsToAvatarParams.js`. All body constraints and relational rules validated.
-   **Semi-Realistic Posture Layer + Release Freeze (Phase 27):** Adds geometry-level posture cues to `computeBodyGeometry` in `HumanAvatarRenderer.jsx`: (A) Shoulder roll — negative postureLean rolls shoulders inward (×3.5px), positive opens outward (×1.5px). (B) Head/neck alignment — negative postureLean shifts head forward (×2.5) and compresses neck height (×1.8px shorter); positive extends neck and centers head. (C) Chest openness — postureLean adjusts effective chestSize ±0.04. (D) Neck path angled via `headFwdShift` so neck visually tilts forward for poor posture. (E) Animation offset/rotate reduced (offset ×2, rotate ×1.5) since geometry now carries the visual load. (F) PostureLean mapping updated — ITE path uses `conf×0.40 + disc×0.35 + emo×0.25`; fallback uses `actNorm×0.35 + nutNorm×0.35 + stressCalm×0.30`. This means fit-but-stressed users have good body but tense posture, and calm-but-soft users have relaxed posture without forced uprightness. (G) MiniAvatarPreview uses same geometry with static `translate` (no motion.g), no overflow. All hair styles validated.
-   **Photo Mode Transformation Layer (Phase 28):** Unified photo overlay system. (A) Created `PhotoTransformLayer` component replacing both `PhotoEffectsLayer` (Current) and `PhotoFutureOverlay` (Future). (B) `computePhotoOverlayState` updated: accepts `rawMetrics` fallback and `postureLean` for framing; produces saturation, brightness, contrast, warmth, vignette, skinGlow, underEye, and framing (scale/translateY/rotate) tied to posture. (C) Current photo now uses trait-based overlays via `computePhotoOverlayState(ite, false, rawMetrics, pl)` — previously used crude `PhotoEffectsLayer`. (D) Visibility floor: when trait deltas exceed 5 points, saturation and brightness are guaranteed at least ±0.03/0.02 so the difference is perceptible. (E) Legacy photo overlays removed — `getGlowOverlayStyle`, `getDarknessOverlayStyle`, animated glow divs, and particle effects no longer render on photos. (F) Posture framing: upright posture slightly scales up and centers the photo; collapsed posture scales down and shifts lower. (G) Confidence gating preserved — Future projection overlay values respect ITE projection confidence tiers; Current uses direct mapping without gating.
-   **Avatar Body Responsiveness Debug + Correction (Phase 29):** Fixed critical pipeline bug where all accounts rendered identical body geometry. (A) Root cause: `computeAvatarEffects()` consumed raw metric scores (activityScore, nutritionScore, etc.) but did NOT include them on the returned object. `mapFromAvatarEffects` then read `avatarEffects?.activityScore` → `undefined` → defaulted to `3` for every metric, every account → identical `PhysicalCompositionScore=50` → identical AverageFit tier. (B) Fix: `computeAvatarEffects` return object now includes `activityScore`, `nutritionScore`, `sleepScore`, `stressScore`, `consistencyScore` so downstream body mapping receives actual per-account values. (C) Dev-only `AvatarDiagnosticsPanel` component added, toggled via `?debugAvatar=1` query param, showing: source flags (ITE/fallback), raw scores, PhysicalCompositionScore, selected tier, interpolation factor, and final body geometry params (shoulderWidth, chestSize, waistTaper, hipWidth, armThickness, legThickness, postureLean, facialTension). Also highlights any fields that defaulted due to undefined values. (D) localStorage namespace fix: `SkinToneSelector` keys (`futureme_skin_tone`, `futureme_hair_style`, `futureme_hair_color`) now namespaced by user ID via `setStorageUserId(uid)` called from `AuthContext` on auth state change, preventing avatar appearance from leaking between accounts. (E) Validated: Excellent account (act=5,nut=5) → PhysScore=95, MuscularAthletic; Poor account (act=1,nut=1) → PhysScore=5, Overweight; Mixed (act=4,nut=2) → PhysScore=53.8, AverageFit.
